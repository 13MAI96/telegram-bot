...
- [x] 1.1 Add configuration for the public wake-up URL and validate that the application can build a root-path wake link from it.
...
- [x] 1.3 Implement an in-memory tracker for inbound HTTP activity and idle-cycle warning state.
...
- [x] 2.3 Record inbound HTTP activity from the web layer and reset the idle-cycle warning state when new traffic starts a new cycle.
...
- [x] 3.1 Define the warning message copy in Spanish and first person, including the wake-up link on `/` and the approved sleep-warning wording.
- [x] 3.2 Add focused tests for recent-user filtering, duplicate-warning suppression, health-check-driven HTTP activity resets, and non-fatal send failures.

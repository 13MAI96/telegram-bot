...
The bot already identifies users by Telegram `from.id` and resolves group membership through MongoDB, but there is no dedicated persistence for “recently active users” or “last warning sent”. The current application already exposes HTTP endpoints and can respond on `/`, so wake-up can be anchored to the deployed public root URL.
...
- Generate the wake-up link from a configured public base URL and direct users to `/`.
...
The warning message should include a URL built from a single environment variable such as `PUBLIC_BASE_URL`, normalized to the root path `/`.
...
### 4. Consider any inbound HTTP request as wake-cycle activity
Any inbound HTTP request should refresh the service activity timestamp and start a new idle cycle if a warning had already been sent for the previous one. This includes visits to `/`, health checks, or other public routes.

Alternative considered:
- Count only explicit `/wake` requests.
  Rejected because Koyeb wake-up can happen through the root route and the user already wants the wake link on `/`.
...
### 6. Treat health-check traffic as legitimate cycle activity for now
For the current implementation, any inbound HTTP request, including platform health checks, should count as legitimate activity for the idle cycle. This matches the current operational assumption that Koyeb may count those requests as inbound traffic for sleep purposes, and it keeps the warning logic aligned with what the platform likely observes.

Alternative considered:
- Ignore health-check requests in the activity clock.
  Rejected for now because it could drift away from Koyeb's actual sleep decision if Koyeb counts those requests.
...
- [HTTP health checks may count as activity and postpone warnings] -> Accept this for now and document that any inbound traffic, including health checks, may extend the current idle cycle.
...
3. Wire HTTP activity recording into the web layer and expose or reuse a root route that can wake the service.

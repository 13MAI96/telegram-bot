...
Hola Espe, Podrias enviar el primer archivo a procesar?
    ...
    async handleDoc(@Ctx() ctx: Scenes.WizardContext) {
        ...
        if (
            ...
            await ctx.reply(
                `Recibi un archivo excel, dejame ver que puedo hacer`,
            );
...
2. Detallado: te doy los totales, pero agrego cada uno de los items que contemple incluyendo el mes de transaccion`,
        ...
        try {
            ...
            await ctx.reply(
                'CSV recibido! Ya lo analice, tenes que sumar algun otro CSV? Mandame el siguiente archivo o listo para recibir el resultado.',
            );
            await ctx.wizard.selectStep(1);
        } catch (error) {
            await ctx.reply(
                `No pude recuperar el archivo. Podrias enviarlo otra vez?`,
            );
    ...
    async step6(@Ctx() ctx: Scenes.WizardContext) {
        if (ctx.message) {
            ...
            switch (option) {
                case 1:
                    await ctx.reply(
                        `Oka, dame unos segundos y te dejo el archivo.`,
                    );
                    const generalReport =
                        await this.excelService.createExcelReport(obj);
                    await ctx.replyWithDocument({
                ...
                case 2:
                    ...
                    const detailedReport =
                        await this.excelService.createDetailedReport(obj);
                    await ctx.replyWithDocument({

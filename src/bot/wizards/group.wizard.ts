...
¿Ya la tienes? (Si/No)
    ...
    async noFile(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            'Perdon, pero no podemos avanzar sin este paso. Aguardo a que realice la copia.',
        );
    }
...
\nSon correctas? 
    ...
    async step8no(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            'Okay, vamos otra vez. Podrias enviarme la lista de categorias separadas por una coma y un espacio?',
        );
...
\nSon correctas? 
    ...
    async step10no(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            'Okay, vamos otra vez. Podrias enviarme la lista de cuentas separadas por una coma y un espacio?',
        );
        await ctx.wizard.back();
    }
...
\nSon correctos? 
    ...
    async step13no(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            'Okay, vamos otra vez. Podrias enviarme los nombres, incluyendo el tuyo, separadas por una coma y un espacio?',
        );
        await ctx.wizard.back();
    }
...
${categories.map((x, index) => `${index}. ${x}`)}
    ...
    async step17(@Ctx() ctx: Scenes.WizardContext) {
        if (ctx.message) {
            ...
            const categories: string[] = ctx.wizard.state['categories'];
            if (selected && selected >= 0 && selected < categories.length) {
                ctx.wizard.state['self_transfer_category'] =
                    categories[selected];
                await ctx.reply(
                    `

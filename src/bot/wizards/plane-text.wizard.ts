...
export class PlaneTextWizard {
    ...
    async step1(@Ctx() ctx: Scenes.WizardContext) {
        ...
        const category = group.categories.find(
            (x) => x.toLowerCase() == bill[1].toLowerCase(),
        );
        const account = group.accounts.find(
            (x) => x.toLowerCase() == bill[3].toLowerCase(),
        );
        const holder = group.holders.find(
            (x) => x.toLowerCase() == bill[4].toLowerCase(),
        );
        if (!category || !account || !holder) {
            await ctx.reply(
                '🚫 Datos invalidos. Recorda enviarme: descripcion, categoria, monto, cuenta y titular separados por una coma.',
            );
            return;
        }
        ...
        ctx.wizard.state['date'] = this.dateService.formatDateToDDMMYYYY(
            new Date(),
        );
...
¿Deseás confirmar? (sí/no) 
    ...
    async reject(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            '❌ Bueno elimino estos datos. Empezamos otra vez? Recorda enviarme: descripcion, categoria, monto, cuenta y titular separados por una coma.',
        );

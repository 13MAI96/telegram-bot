        ...
        if (ctx.message && ctx.message['text']) {
            ...
            if (action) {
                switch (action) {
                    case 1:
                        await ctx.reply(
                            `Que categorias deseas agregar? Envialas separadas por coma.`,
                        );
                        await ctx.wizard.next();
                        break;
                    case 2:
                        const categories: string[] =
                            ctx.wizard.state['group'].categories;
                        await ctx.reply(`
...
\nCual de ellas deseas eliminar?
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        const instalment_categories: string[] =
                            ctx.wizard.state['group'].instalment_categories;
                        await ctx.reply(`
Estas son las categorias actuales:
...
\nSon correctas? 
    ...
    async step4(@Ctx() ctx: Scenes.WizardContext) {
        await ctx.reply(
            'Okay, vamos otra vez. Enviame las categorias separadas por coma:',
        );

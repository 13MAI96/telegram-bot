...
export class ExcelService {
        ...
        try {
            ...
            workbook.eachSheet((worksheet, sheetId) => {
                ...
                worksheet.eachRow((row, rowNumber) => {
                    ...
                    const conceptValue = this.toText(values[1]);

                    if (!Array.isArray(values) || !conceptValue) return;

                    const concept: DetailedConcept = {
                        concept: ``,
                        count: 0,
                        debitos: 0,
                        creditos: 0,
            ...
            concepts.sort((a, b) =>
                a.concept.toLowerCase().localeCompare(b.concept.toLowerCase()),
            );
    ...
    async createExcelReport(concepts: DetailedConcept[]): Promise<string> {
        ...
        concepts.forEach((x) => {
            const concept = result.find(
                (y) =>
                    `${y.concept.toUpperCase()}` ==
                    `${x.concept.toUpperCase()}`,
            );
            if (concept) {
                concept.count++;
                concept.debitos = this.toNumber(
                    concept.debitos + this.toNumber(x.debitos),
                );
                concept.creditos = this.toNumber(
                    concept.creditos + this.toNumber(x.creditos),
                );
                concept.balance = this.toNumber(
                    concept.balance -
                        this.toNumber(x.debitos) +
                        this.toNumber(x.creditos),
                );
            } else {
                result.push({
                    concept: `${x.concept}`,
                    ...
                    balance:
                        this.toNumber(x.creditos) - this.toNumber(x.debitos),
                });
            }
        });
        ...
        const rowsToArray: any[][] = [
            ['Concepto', 'Count', 'DEBE acum', 'HABER acum', 'Balance'],
        ];
    ...
    public async createDetailedReport(concepts: DetailedConcept[]) {
        ...
        const rowsToArray: any[][] = [
            ['Concepto', 'Mes', 'DEBE acum', 'HABER acum', 'Balance'],
        ];
        for (const row of result) {
            rowsToArray.push([
                row.concept,
